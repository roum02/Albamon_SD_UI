...
안녕하세요!
알바몬제트개발팀의 김규리입니다.
오늘은 Server Driven UI PoC를 진행하며 GraphQL, Apollo Server를 도입하게 된 과정을 공유하고자 합니다.
가볍게 읽어주시면 감사합니다.
...
예를 들어, 프론트엔드는 상품의 가격 정보만 있으면 되는데 백엔드에서 상품의 모든 정보와 판매자 및 구매자의 정보까지 모조리 줬습니다.

처음에는 체감상 큰 문제는 없어서 방치했습니다.. 그러나 해당 API는 다른 기능에서도 사용되고 있었기에 요구사항에 따라 변경되었고, 이에 따른 JOIN 쿼리가 많아지면서 문제가 생기기 시작했습니다.
...
모르는 것은 모른다고 말하고 모르는 것을 알아가는 것이 개발자의 삶이니까(?) 그냥 넘어갔습니다.

이때부터 GraphQL은 완수하지 못한 과제처럼 마음 한구석에 남아있었던 것 같습니다.

만 2년의 시간이 흐르고 3년차가 된 지금,
지금 회사에서 사수님께서 SDUI(Server Driven UI) 라는 기술을 이야기해주셨습니다.
...
우리 팀은 알바몬에서 SDUI가 필요한 상황을 재현해보고 PoC(Proof of Concept)를 진행했습니다.
...
GraphQL을 사용하는 회사로 대표적으로 Airbnb가 있는데 아래 그림을 한 번 봐보겠습니다.
...
GraphQL이 뭔지는 대강 보여드린것 같으니, 이 쿼리 언어를 사용하기 위해서는 필요한 서버를 구축해보겠습니다.
...
우리도 앞으로 세계로 뻗어나갈 글로벌 서비스를 해야하니 (~~회사에서그런말한적없음~~)
세계적 탑 개발자~~라고 생각하는~~ 우리가 선택하기엔 적절한 기술임이 분명합니다.
아무튼 대규모 서비스의 기업에서 프로덕션까지 올렸으니 신뢰해도 좋다고 생각했습니다.
