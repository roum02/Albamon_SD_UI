...
"아 진짜 이 API에서 필요한 데이터만 받아올 수 있으면 좋겠다...☹️"
...
1. **오버패칭 문제 해결**: 클라이언트가 필요한 데이터만 요청할 수 있어서 불필요한 데이터 전송을 막을 수 있습니다.
2. **유연한 데이터 요청**: 하나의 엔드포인트로 여러 종류의 데이터를 원하는 형태로 받아올 수 있습니다.
3. **타입 시스템**: 강력한 타입 체크로 런타임 에러를 줄일 수 있습니다.
...
만 2년의 시간이 흐르고 3년차가 된 지금, 지금의 회사에 오게되었습니다.

우리 팀에서는 이벤트 페이지를 개발할 때마다 새로운 컴포넌트들을 만들고 있었습니다.
살펴보니 대부분의 이벤트 페이지들이 타이틀, 이미지, 버튼 등 비슷한 구조를 가지고 있었기 때문에 팀 내에서 고민이 있었습니다.

"매번 비슷한 컴포넌트를 새로 만드는게 비효율적이지 않나..."
"이벤트 페이지 구조를 좀 더 체계적으로 관리할 수 없을까?"

이런 고민을 팀원들과 나누던 중, 사수님께서 Server Driven UI라는 개념을 소개해주셨습니다. UI 구조를 서버에서 제어하면 프론트엔드 배포 없이도 유연하게 화면을 구성할 수 있고, 컴포넌트를 정형화하여 재사용성을 높일 수 있다는 것이 었습니다. 이렇게 하면 매번 새로운 이벤트 페이지를 만들 때마다 비슷한 컴포넌트를 반복해서 개발하는 문제도 해결할 수 있을 것 같았습니다.
...
그런데 말입니다... 쿼리를 이제 프론트에서 짜야한다고? 너무 귀찮지 않을까? 하는 생각이 들었습니다.
하지만 Apollo Server에서 제공하는 Playground를 사용하면 쿼리를 자동으로 짜줍니다. (이건 Yoga도 지원하긴 함)

플레이그라운드는 GraphQL 쿼리를 쉽게 작성하고 테스트할 수 있는 도구입니다. 이를 통해 개발자는 복잡한 쿼리를 손쉽게 생성하고 시뮬레이션을 해볼 수 있습니다!
...
이번 파일럿을 통해 GraphQL과 Apollo Server를 성공적으로 도입해보았지만, 이제 진짜 시작입니다.

![화이트보드에 SDUI 구조를 그리며 고민하는 팀원들]
